...
const translations = {
    ...
    bg: {
        categories: '╨Ъ╨░╤В╨╡╨│╨╛╤А╨╕╨╕ ╨Ь╨╡╨╜╤О',
        searchPlaceholder: '╨в╤К╤А╤Б╨╡╨╜╨╡ ╨╜╨░ ╤П╤Б╤В╨╕╤П...',
        allItems: '╨Т╨б╨Ш╨з╨Ъ╨Ш ╨Я╨а╨Ю╨Ф╨г╨Ъ╨в╨Ш',
        noResults: '╨Э╨╡ ╤Б╨░ ╨╜╨░╨╝╨╡╤А╨╡╨╜╨╕ ╨┐╤А╨╛╨┤╤Г╨║╤В╨╕',
        addToCart: '╨Ф╨╛╨▒╨░╨▓╨╕',
        promo: '╨Я╨а╨Ю╨Ь╨Ю',
        bundle: '╨Ъ╨Ю╨Ь╨С╨Ю',
        save: '╨б╨Я╨Х╨б╨в╨Ш'
...
function formatPrice(priceEUR) {
    ...
    if (currencySettings.showBgnPrices) {
        return `<span class="price-bgn">${priceBGN} ╨╗╨▓</span> <span class="price-separator">/</span> <span class="price-eur">тВм${priceEUR.toFixed(2)}</span>`;
    } else {
        return `<span class="price-eur">тВм${priceEUR.toFixed(2)}</span>`;
    }
}
...
function createProductCard(product) {
    ...
    card.setAttribute('data-product-id', product.id); // Add data attribute for scroll targeting
...
function renderSearchResults(results, query) {
    console.log('ЁЯУЛ Rendering search results:', results.length, 'items');
    const dropdown = document.getElementById('search-results');
    
    if (!dropdown) {
        console.error('тЭМ Dropdown element not found!');
        return;
    }
    
    if (results.length === 0) {
        dropdown.innerHTML = `<div class="search-no-results">${currentLanguage === 'bg' ? '╨Э╤П╨╝╨░ ╨╜╨░╨╝╨╡╤А╨╡╨╜╨╕ ╨┐╤А╨╛╨┤╤Г╨║╤В╨╕' : 'No products found'}</div>`;
        dropdown.classList.add('show');
        console.log('тЬЕ Showing "no results" message');
        return;
    }
    ...
    dropdown.innerHTML = results.map(product => {
        ...
        console.log('ЁЯУж Product:', name, 'Category:', category, 'Price:', product.price);
        
        return `
            <div class="search-result-item" onclick="scrollToProduct(${product.id})">
                ${product.image ? `<img src="${product.image}" class="search-result-img" alt="${name}">` : ''}
                <div class="search-result-info">
                    <div class="search-result-name">${name}</div>
                    <div class="search-result-category">${category}</div>
                </div>
                <div class="search-result-price">${product.price.toFixed(2)} ${currentLanguage === 'bg' ? '╨╗╨▓' : 'BGN'}</div>
            </div>
        `;
    }).join('');
    
    dropdown.classList.add('show');
    console.log('тЬЕ Dropdown shown with', results.length, 'results');
    console.log('Dropdown classes:', dropdown.className);
    console.log('Dropdown display:', window.getComputedStyle(dropdown).display);
    console.log('Dropdown height:', window.getComputedStyle(dropdown).height);
    ...
    if (firstItem) {
        console.log('First item display:', window.getComputedStyle(firstItem).display);
        console.log('First item height:', window.getComputedStyle(firstItem).height);
        console.log('First item visibility:', window.getComputedStyle(firstItem).visibility);
    }
}

// Scroll to product and pulse
function scrollToProduct(productId) {
    // Close search dropdown and mobile overlay
    ...
    if (productCard) {
        ...
        }, 1800); // 3 pulses ├Ч 0.6s = 1.8s
    }
}

// Mobile search toggle functions
function toggleMobileSearch() {
    console.log('ЁЯФН Toggle mobile search clicked');
    const container = document.getElementById('search-container');
    const input = document.getElementById('search-input');
    
    if (!container) {
        console.error('тЭМ Search container not found!');
        return;
    }
    
    console.log('тЬЕ Adding active class to container');
...
function closeMobileSearch() {
    console.log('тЭМ Close mobile search clicked');
    const container = document.getElementById('search-container');
    const input = document.getElementById('search-input');
    const dropdown = document.getElementById('search-results');
    ...
    // Trigger products re-render to show all
    renderProducts();
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    searchInput.addEventListener('input', function(e) {
        ...
        console.log('ЁЯФд Search input:', query);
        
        const isMobile = window.innerWidth <= 768;
        const searchContainer = document.getElementById('search-container');
        const isMobileOverlay = isMobile && searchContainer.classList.contains('active');
        
        console.log('ЁЯУ▒ Is mobile overlay:', isMobileOverlay);
        
        // Only filter products in main view if NOT in mobile overlay
        if (!isMobileOverlay) {
            renderProducts();
        }
        
        // Show autocomplete dropdown after 300ms
        searchTimeout = setTimeout(() => {
            console.log('тП▒я╕П Timeout fired, query length:', query.length);
            
            if (query.length < 2) {
                console.log('тЭМ Query too short, hiding dropdown');
                document.getElementById('search-results').classList.remove('show');
                return;
            }
            
            console.log('ЁЯФН Searching products...');
            const results = products.filter(p => {
                ...
                return nameEN.includes(query) || 
                       nameBG.includes(query) ||
                       descriptionEN.includes(query) ||
                       descriptionBG.includes(query);
            }).slice(0, 8);
            
            console.log('тЬЕ Found', results.length, 'results');
    ...
    document.addEventListener('click', function(e) {
        if (!e.target.closest('.search-bar-top')) {
            document.getElementById('search-results').classList.remove('show');
        }
    });
    ...
    window.onclick = function(event) {
        ...
        if (searchContainer && searchContainer.classList.contains('active')) {
            if (!event.target.closest('#search-container') && !event.target.closest('.search-toggle-mobile')) {
                closeMobileSearch();
            }
        }
    };
